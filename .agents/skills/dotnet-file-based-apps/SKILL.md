...
References another cs file . Use this to share code between a file-based app and a traditional project.

```csharp
#:include ../SharedLibrary/*.cs
```

The referenced project is built and linked as a project reference, just like `<Compile >` in a `.cs`.
